-- MailoReply AI - Stripe Integration Schema Extension (CONFLICT-FREE VERSION)
-- Adds subscription management and payment tracking to the existing clean schema
-- This version removes function conflicts and ensures clean deployment
...
DO $$ 
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_type WHERE typname = 'price_interval') THEN
...
-- RLS Policies for subscription data (drop if exists, then create)
...
-- Extension usage tracking function (simplified to avoid conflicts)
...
BEGIN
  ...
  -- Using a safer approach that won't conflict with existing functions
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_user_subscriptions_user') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_payment_history_user') THEN
    CREATE INDEX idx_payment_history_user ON public.payment_history(user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_indexes WHERE indexname = 'idx_payment_history_subscription') THEN
...
BEGIN
  ...
  RAISE NOTICE '1. Update your Stripe product IDs in the stripe_products table';
  RAISE NOTICE '2. Update your Stripe price IDs in the stripe_prices table';
  RAISE NOTICE '3. Configure webhook endpoints in your Stripe dashboard';
  RAISE NOTICE '4. Test the subscription flow end-to-end';
END $$;