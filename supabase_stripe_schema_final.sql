-- MailoReply AI - Stripe Integration Schema Extension (FINAL CORRECTED VERSION)
-- Adds subscription management and payment tracking to the existing clean schema
-- This version fixes PostgreSQL system table column name issues
...
-- Enable RLS on new tables (simplified approach - no system table checks)
...
BEGIN
  ...
  -- Increment user usage counters (if the function exists)
  BEGIN
...
-- Create indexes for performance (using simpler approach)
...
-- Success message
SELECT 'Stripe integration schema deployed successfully! ✅' as result;
SELECT 'Next steps:' as info;
SELECT '1. Update your Stripe product IDs in the stripe_products table' as step1;
SELECT '2. Update your Stripe price IDs in the stripe_prices table' as step2;
SELECT '3. Configure webhook endpoints in your Stripe dashboard' as step3;
SELECT '4. Test the subscription flow end-to-end' as step4;